...
echo "$($PSQL "SELECT name FROM games LEFT JOIN teams ON games.opponent_id=teams.team_id WHERE round='Eighth-Final' AND year=2014 UNION SELECT name FROM games LEFT JOIN teams ON games.winner_id=teams.team_id WHERE round='Eighth-Final' AND year=2014;")"
